    ...
    makeQuadTree(imageData, context, blockSize, circleBool, timeoutType = '1', byVar = false, ratio = 1) {
        ...
        this.Quadtree = class Quadtree {
            ...
            split(force = false) {
                ...
                devisions++;
                divisionsNumberEl.innerText = `division: ${devisions}\n bottom-level-nodes:${1 + 3 * devisions}`;
                return this;
            }
            splitChildren() {
                ...
                if (QuadNode.split()) {
                    QuadNode.nodes.forEach(function (node, index) {
                        if (node.nextWidth >= blockSize && node.nextWidth >= 1 && node.nextHeight >= 1) {
                            if (timeoutType === '2') {
                                timeOutes.push(setTimeout(() => node.recusiveSplit(node), (node.level * index) * 100));
                            } else if (timeoutType === '1') {
                                timeOutes.push(setTimeout(() => node.recusiveSplit(node), 10));
                            } else if (timeoutType === '3') {
                                timeOutes.push(setTimeout(() => node.recusiveSplit(node), ((100) / (node.level * index * index))));
                            } else if (timeoutType === '4') {
